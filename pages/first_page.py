    ...
    try:
      ...
      for region in regions:
          ...
          for concept in region.data.concepts:
              ...
              # Top-left corner
              cv2.line(_frame, (int(left_col * frame.shape[1]), int(top_row * frame.shape[0])),
                    (int(left_col * frame.shape[1]) + corner_length, int(top_row * frame.shape[0])), (0, 255, 0), 2)
              cv2.line(_frame, (int(left_col * frame.shape[1]), int(top_row * frame.shape[0])),
                    (int(left_col * frame.shape[1]), int(top_row * frame.shape[0]) + corner_length), (0, 255, 0), 2)

              # Top-right corner
              cv2.line(_frame, (int(right_col * frame.shape[1]), int(top_row * frame.shape[0])),
                    (int(right_col * frame.shape[1]) - corner_length, int(top_row * frame.shape[0])), (0, 255, 0), 2)
              cv2.line(_frame, (int(right_col * frame.shape[1]), int(top_row * frame.shape[0])),
                    (int(right_col * frame.shape[1]), int(top_row * frame.shape[0]) + corner_length), (0, 255, 0), 2)

              # Bottom-left corner
              cv2.line(_frame, (int(left_col * frame.shape[1]), int(bottom_row * frame.shape[0])),
                    (int(left_col * frame.shape[1]) + corner_length, int(bottom_row * frame.shape[0])), (0, 255, 0), 2)
              cv2.line(_frame, (int(left_col * frame.shape[1]), int(bottom_row * frame.shape[0])),
                    (int(left_col * frame.shape[1]), int(bottom_row * frame.shape[0]) - corner_length), (0, 255, 0), 2)

              # Bottom-right corner
              cv2.line(_frame, (int(right_col * frame.shape[1]), int(bottom_row * frame.shape[0])),
                    (int(right_col * frame.shape[1]) - corner_length, int(bottom_row * frame.shape[0])), (0, 255, 0), 2)
              cv2.line(_frame, (int(right_col * frame.shape[1]), int(bottom_row * frame.shape[0])),
                    (int(right_col * frame.shape[1]), int(bottom_row * frame.shape[0]) - corner_length), (0, 255, 0), 2)
